...
func analyze_obj(fn string, db *sql.DB) {
	r2p, err := r2.NewPipe(fn)
	if err != nil {
		panic(err)
		}
	init_fw(r2p)
	funcs_data := get_all_funcdata(r2p)
	relocs_data := get_all_relocdata(r2p)

	for _,f :=range funcs_data {
	//	fmt.Println(strings.ReplaceAll(f.Name, "sym.", ""))
		s, err := get_f_relocs(f.Name, relocs_data, funcs_data)
		if err!=nil{
			panic(err)
			}
		Insert_data(db, strings.ReplaceAll(f.Name, "sym.", ""), fn, s, false)
	//	fmt.Println(s)
		}
